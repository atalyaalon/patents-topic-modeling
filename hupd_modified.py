    ...
    def _split_generators(self, dl_manager: datasets.DownloadManager):
        ...
        if self.config.force_extract:
            if self.config.name == 'all':
                ...
                import tarfile
                fields_to_keep = {"application_number", "title", "abstract", "date_published", "filing_date"}
                for year in range(2004, 2019):
                    tar_file_path = f'{json_dir}/{year}.tar.gz'
                    if year in full_year_range:
                        with tarfile.open(tar_file_path) as tar_file:
                            print(f'Extracting {tar_file_path}')
                            tar_file.extractall(f'{json_dir}')
                            
                            # Process each JSON in the archive
                            for member in tar_file.getmembers():
                                if member.isfile() and member.name.endswith('.json'):
                                    file_path = os.path.join(json_dir, member.name)
                                    try:
                                        with open(file_path, 'r') as f:
                                            data = json.load(f)
                                        # Keep only relevant fields if they exist
                                        filtered = {k: v for k, v in data.items() if k in fields_to_keep}
                                        with open(file_path, 'w') as f:
                                            json.dump(filtered, f, indent=2)
                                    except Exception as e:
                                        print(f"Error processing {file_path}: {e}")
                            print(f'Extracted and modified fields from {tar_file_path}')
